...
void VF2::GenRevGraph(const VF2Graph &src, VF2Graph &dst)
{
	dst = VF2Graph();
...
{
	// Update core,in,out
	//abeyn//for (int i = 0; i < pat.vn; i++)
	{
		s.core1[a] = b;
		s.in1[a] = 0;
		s.out1[a] = 0;
	}
	//abeyn//for (int i = 0; i < g.vn; i++)
	{
		s.core2[b] = a;
		s.in2[b] = 0;
		s.out2[b] = 0;
...
void VF2::convertToVF2(Graph& graph, VF2Graph& graphVF2)
{
	graphVF2.init();
	...
	for (auto node : graph._nodes)
	{
		...
		{
			int kind = edgeList._kind;
			for (auto &edge : edgeList._edges)
			{
				int n1		= (int)edge._n1;
				int n2		= (int)edge._n2;
				int val		= (edge._val == Logic::True ? 1 : 0);
				//генерируем метку для ребра графа
				int label   = kind * 2 + val;
				graphVF2.addse(n1 - 1, n2 - 1, label);
			}
		}
	}
}

bool VF2::isomorphic(Graph* queryGraph, Graph* mainGraph)
{
	//конвертируем графы в формат VF2
	convertToVF2(*queryGraph, pat);
	convertToVF2(*mainGraph, g);
